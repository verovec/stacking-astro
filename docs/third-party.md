...
| **StarNet++ v2** | *Optional.* Star removal for star-reduced finishing. Absent → full stars. **Licence is not redistributable**, which is why the Docker engine image does not bake it in. Upstream also ships **no linux/arm64 build**, so an Apple-Silicon container reaches the host install over HTTP (`cmd/starnet-host`) rather than by bind-mount. | see upstream (starnetastro.com) | `STARNET_BIN`, `STARNET_CLI`, `ASTRO_STARNET_URL` |
